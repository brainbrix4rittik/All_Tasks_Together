...
3.  **Locations/Settings:** Use `rectangle` shapes or distinct `text` blocks.
    * **Labels:** Use a `text` tool call with the location name and relevant description (e.g., "Setting: Primary Location", "Location: Specific Place"). Position the label clearly, preferably centered within or directly below the shape/block. Ensure the shape/block is large enough to fit the text with padding.
...
6.  **Events/Plot Points:** Use distinct `text` blocks or shapes (like rectangles or circles) with associated text labels.
    * **Labels:** Use a `text` tool call describing the event clearly (e.g., "Event: Key Moment", "Plot Point: Turning Point"). Position the label clearly, preferably centered within or directly below the shape/block. Ensure the shape/block is large enough to fit the text with padding.
